...
public class EnquiryAddEditDeleteTest extends base {
	...
	//	ExcelUtils excel = new ExcelUtils(dataExcelPath + "/TestDataExcel/ZylerERPTestDataExcel.xlsx", "Enquiry");		
		...
		ep.getCustomerDropdownSearch().sendKeys(excel.getCellDataString(3, 0));
		Thread.sleep(5000);
		...
		ep.getAddItemDropdownSearch().sendKeys(excel.getCellDataString(3, 1));
		Thread.sleep(4000);
		...
		//Add Lead Time 
		ep.getLeadTime().sendKeys(excel.getCellDataNumber(3, 3));
		...
		//Add Quantity code
		ep.getQty().clear();
		ep.getQty().sendKeys(excel.getCellDataNumber(3, 2));
		
		//comments
		ep.getComments().sendKeys(excel.getCellDataString(3, 8));
...
//		System.out.println("First product_name from the list ==> "+ product_name);
	...
	@Test(priority=2)
	public void edit_existing_Enquiry() throws IOException,InterruptedException
	{
		ep = new EnquiriesPage(driver);
		
		ep.getSearch().sendKeys(enquiryNumber);
		Thread.sleep(2000);
		ep.getEnquiryIDTable().click();
		//Thread.sleep(5000);
		driver.switchTo().activeElement();
		Thread.sleep(2000);
		ep.getEditEnquiryButton().click();
		...
		// Lead Time 
		ep.getLeadTime().clear();
		ep.getLeadTime().sendKeys(excel.getCellDataNumber(2, 3));
		
		//Due Date
		ep.getDueDate().click();
		ep.getEditDueDateNextButton().click();
		ep.getEditDueDateNextButton().click();
		Thread.sleep(2000);
		driver.switchTo().activeElement();
		ep.getDueDateSelect().click();
		...
		ep.getTargetPrice().sendKeys(excel.getCellDataNumber(2, 4));
		
		//comments
		ep.getComments().clear();
		ep.getComments().sendKeys(excel.getCellDataString(2, 8));
		
		//Country
		ep.getCountryDropdown().click();
		Thread.sleep(2000);
		//ep.getCountryDropdownSearch().sendKeys(excel.getCellDataString(2, 7));
		List<WebElement> countryList = ep.getCountryDropdownlist();//driver.findElements(By.xpath("//ul[@class='dropdown-menu inner show']//li"));
		for(int i=0 ; i<countryList.size(); i++) {
			String list_country = countryList.get(i).getText().replace(" -", "");
			String Excel_data=excel.getCellDataString(2, 7);
		...
		//Save button
		ep.getEditSaveButton().click();
		
		//Verifying success Message
		WebElement success_wait = new WebDriverWait(driver, Duration.ofSeconds(10)).until(ExpectedConditions.visibilityOf(ep.getSuccessMessage()));	
		String sucess_message = success_wait.getText();
		String expected_success_message = "Enquiry updated Successfully.";
		Assert.assertTrue(sucess_message.contains(expected_success_message));
		System.out.println("Message  :"+sucess_message.replace("×", ""));
	...
	@Test(priority=3)
	public void delete_existing_Enquiry() throws IOException,InterruptedException
	{
		ep = new EnquiriesPage(driver);
		...
		driver.switchTo().activeElement();
		Thread.sleep(2000);
		ep.getMoreButton().click();
		
		ep.getDeleteButton().click();
		...
		WebElement success_wait = new WebDriverWait(driver, Duration.ofSeconds(10)).until(ExpectedConditions.visibilityOf(ep.getSuccessMessage()));	
		String sucess_message = success_wait.getText();
		String expected_success_message = "Enquiry deleted Successfully";
		Assert.assertTrue(sucess_message.contains(expected_success_message));
		System.out.println("Message  :"+sucess_message.replace("×", ""));
